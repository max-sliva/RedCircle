...
class RangeSliderUI extends BasicSliderUI {
     ...
     * @param b RangeSlider
     */
    public RangeSliderUI(RangeSliderOld b) {
        super(b);
    }
...
//        Ellipse2D shape = new Ellipse2D.Double(0, 0, width, height);
        int[] xPoints = {0, width, width / 2}; // X coordinates
        int[] yPoints = {1, 1, height};   // Y coordinates
...
//        Polygon triangle = new Polygon(xPoints, yPoints, nPoints);
        ...
        myPath.lineTo(xPoints[1], yPoints[1]);
        myPath.lineTo(xPoints[2], yPoints[2]);
        myPath.closePath();
        return myPath;
//        return shape;
    }
    ...
    public void scrollByBlock(int direction) {
        synchronized (slider) {
            ...
            if (upperThumbSelected) {
                int oldValue = ((RangeSliderOld) slider).getUpperValue();
                ((RangeSliderOld) slider).setUpperValue(oldValue + delta);
            } else {
